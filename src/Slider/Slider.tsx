...
const Button = styled.button<{ isDisabled: boolean }>`
  ...
  &:before {
    background-color: ${(props) => (props.isDisabled ? '#505050' : 'transparent')};
    content: '';
    position: absolute;
    top: 0;
    left: 0;
...
const Slider: FC<{ children: ReactNode[]; title: string }> = ({ children, title }) => {
  ...
  useEffect(() => {
    const firstSlideNode = trackRef.current?.firstChild
    const lastSlideNode = trackRef.current?.lastChild
    const hasIOSupport = !!window.IntersectionObserver
    let isMounted = true

    if (!hasIOSupport || !firstSlideNode || !lastSlideNode) return
    ...
    const updateEntry = ([entry]: IntersectionObserverEntry[]): void => {
      ...
      if (entry.isIntersecting) {
        if (entry.target === firstSlideNode) {
    ...
    const observerParams = { threshold: 1, root: null, rootMargin: '0px' }
    const observer = new IntersectionObserver(updateEntry, observerParams)

    observer.observe(firstSlideNode as Element)
    observer.observe(lastSlideNode as Element)
    ...
    <Root>
      <Container>
        <Header>
          <h2>{title}</h2>
          <Controls>
            <Button onClick={() => scrollSlides(-2)} isDisabled={isFirstSlide}>
            ...
            <Button onClick={() => scrollSlides(2)} isDisabled={isLastSlide}>
              <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
