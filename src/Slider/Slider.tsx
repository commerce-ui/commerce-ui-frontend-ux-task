...
const Slider: FC<{ children: ReactNode[]; title: string }> = ({ children, title }) => {
  ...
  useEffect(() => {
    const elements = trackRef.current?.childNodes;

    if(observerRef.current) observerRef.current.disconnect();
    if(!elements || elements.length <= 1) return;

    const lastItem = elements[elements.length - 1] as Element;
    const firstItem = elements[0] as Element;

    observerRef.current = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        switch (entry.target) {
          case firstItem:
            setIsLeftButtonDisabled(entry.isIntersecting);
            break;
          case lastItem: 
            setIsRightButtonDisabled(entry.isIntersecting);
            break;
        };
      });
    ...
    <Root>
      <Container>
        <Header>
          <h2>{title}</h2>
          <Controls>
            <button onClick={() => scrollSlides(-2)} disabled={isLeftButtonDisabled}>
            ...
            <button onClick={() => scrollSlides(2)} disabled={isRightButtonDisabled}>
              <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
